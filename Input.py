    ...
    try:
        with open(input_file, "r", encoding='UTF8') as f:
            ...
            for task in rt_tasks:
                task.ga_processor_mode = [0 for _ in range(max_core + 1)]
                task.ga_memory_mode = [0 for _ in range(max_core + 1)]

            for core in (max_core, min_core - 1, -1):
                f.readline()
                for task in rt_tasks:
                    line = list(map(int, f.readline().split()))
                    task.ga_processor_mode[core] = line[0]
                    task.ga_memory_mode[core] = line[1]
